...
class UNnet2H(tf.keras.Model):
  ...
  def __init__(self,
    ...
    super(UNnet2H, self).__init__(name=name, **kwargs)
    ...
    # decode_conv6_1
    if self.dropout_rate < 1:
        self.Drop6_1 = Dropout(self.dropout_rate)
    ...
    # decode_conv6_2
    if self.dropout_rate < 1:
        self.Drop6_2 = Dropout(self.dropout_rate)
    ...
    # decode_conv7_1
    if self.dropout_rate < 1:
        self.Drop7_1 = Dropout(self.dropout_rate)
    ...
    # decode_conv7_2
    if self.dropout_rate < 1:
        self.Drop7_2 = Dropout(self.dropout_rate)
...
class UNnetS2H(tf.keras.Model):
  ...
  def __init__(self,
    ...
    super(UNnetS2H, self).__init__(name=name, **kwargs)
    self.dropout_rate = dropout_rate
    self.batch_norm=batch_norm
    # encode_conv1
    self.Conv1_1 = Conv2D(filters, 3, padding='same', kernel_initializer='he_normal')
    ...
    if self.dropout_rate < 1:
        self.Drop4 = Dropout(self.dropout_rate)
    self.Conv4_1 = Conv2D(16 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu4_1 = ReLU()
    self.Conv4_2 = Conv2D(16 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu4_2 = ReLU()
    self.UpSample4 = UpSampling2D(size=(2, 2))
    self.UpConv4 = Conv2D(8 * filters, 1, padding='same', kernel_initializer='he_normal')
    self.ReluUp4 = ReLU()
    if batch_norm:
        self.Norm4_1 = BatchNormalization()
        self.Norm4_2 = BatchNormalization()
    ...
    # decode_conv5_1
    if self.dropout_rate < 1:
        self.Drop5_1 = Dropout(self.dropout_rate)
    self.Merge5_1 = Concatenate(axis=-1)
    self.Conv5_1_1 = Conv2D(8 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu5_1_1 = ReLU()
    self.Conv5_1_2 = Conv2D(8 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu5_1_2 = ReLU()
    self.UpSample5_1 = UpSampling2D(size=(2, 2))
    self.UpConv5_1 = Conv2D(4 * filters, 1, padding='same', kernel_initializer='he_normal')
    self.ReluUp5_1 = ReLU()
    if batch_norm:
        self.Norm5_1_1 = BatchNormalization()
        self.Norm5_1_2 = BatchNormalization()
        self.NormUp5_1 = BatchNormalization()
    # decode_conv5_2
    if self.dropout_rate < 1:
        self.Drop5_2 = Dropout(self.dropout_rate)
    self.Merge5_2 = Concatenate(axis=-1)
    self.Conv5_2_1 = Conv2D(8 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu5_2_1 = ReLU()
    self.Conv5_2_2 = Conv2D(8 * filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu5_2_2 = ReLU()
    self.UpSample5_2 = UpSampling2D(size=(2, 2))
    self.UpConv5_2 = Conv2D(4 * filters, 1, padding='same', kernel_initializer='he_normal')
    self.ReluUp5_2 = ReLU()
    if batch_norm:
        self.Norm5_2_1 = BatchNormalization()
        self.Norm5_2_2 = BatchNormalization()
        self.NormUp5_2 = BatchNormalization()
    # decode_conv6_1
    if self.dropout_rate < 1:
        self.Drop6_1 = Dropout(self.dropout_rate)
    self.Merge6_1 = Concatenate(axis=-1)
    ...
    self.Merge7_1 = Concatenate(axis=-1)
    self.Conv7_1_1 = Conv2D(filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu7_1_1 = ReLU()
    self.Conv7_1_2 = Conv2D(filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu7_1_2 = ReLU()
    if batch_norm:
        self.Norm7_1_1 = BatchNormalization()
        self.Norm7_1_2 = BatchNormalization()
    ...
    self.Merge7_2 = Concatenate(axis=-1)
    self.Conv7_2_1 = Conv2D(filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu7_2_1 = ReLU()
    self.Conv7_2_2 = Conv2D(filters, 3, padding='same', kernel_initializer='he_normal')
    self.Relu7_2_2 = ReLU()
    if batch_norm:
        self.Norm7_2_1 = BatchNormalization()
        self.Norm7_2_2 = BatchNormalization()
  ...
  def call(self, inputs):
    ...
    # conv5_1
    merge5_1 = self.Merge5_1([conv3_2, up4])
    if self.dropout_rate < 1:
        drop5_1 = self.Drop5_1(merge5_1)
    conv5_1_1 = self.Conv5_1_1(drop5_1)
    conv5_1_1 = self.Relu5_1_1(self.Norm5_1_1(conv5_1_1)) if self.batch_norm else self.Relu5_1_1(conv5_1_1)
    conv5_1_2 = self.Conv5_1_2(conv5_1_1)
    conv5_1_2 = self.Relu5_1_2(self.Norm5_1_2(conv5_1_2)) if self.batch_norm else self.Relu5_1_2(conv5_1_2)
    up5_1 = self.UpSample5_1(conv5_1_2)
    up5_1 = self.UpConv5_1(up5_1)
    up5_1 = self.ReluUp5_1(self.NormUp5_1(up5_1)) if self.batch_norm else self.ReluUp5_1(up5_1)
    # conv5_2
    merge5_2 = self.Merge5_2([conv3_2, up4])
    if self.dropout_rate < 1:
        drop5_2 = self.Drop5_2(merge5_2)
    conv5_2_1 = self.Conv5_2_1(drop5_2)
    conv5_2_1 = self.Relu5_2_1(self.Norm5_2_1(conv5_2_1)) if self.batch_norm else self.Relu5_2_1(conv5_2_1)
    conv5_2_2 = self.Conv5_2_2(conv5_2_1)
    conv5_2_2 = self.Relu5_2_2(self.Norm5_2_2(conv5_2_2)) if self.batch_norm else self.Relu5_2_2(conv5_2_2)
    up5_2 = self.UpSample5_2(conv5_2_2)
    up5_2 = self.UpConv5_2(up5_2)
    up5_2 = self.ReluUp5_2(self.NormUp5_2(up5_2)) if self.batch_norm else self.ReluUp5_2(up5_2)
    # conv6_1
    merge6_1 = self.Merge6_1([conv2_2, up5_1])
    if self.dropout_rate < 1:
        drop6_1 = self.Drop6_1(merge6_1)
    conv6_1_1 = self.Conv6_1_1(drop6_1)
    conv6_1_1 = self.Relu6_1_1(self.Norm6_1_1(conv6_1_1)) if self.batch_norm else self.Relu6_1_1(conv6_1_1)
    ...
    up6_1 = self.ReluUp6_1(self.NormUp6_1(up6_1)) if self.batch_norm else self.ReluUp6_1(up6_1)
    # conv6_2
    merge6_2 = self.Merge6_2([conv2_2, up5_2])
    if self.dropout_rate < 1:
        drop6_2 = self.Drop6_2(merge6_2)
    conv6_2_1 = self.Conv6_2_1(drop6_2)
    conv6_2_1 = self.Relu6_2_1(self.Norm6_2_1(conv6_2_1)) if self.batch_norm else self.Relu6_2_1(conv6_2_1)
    ...
    up6_2 = self.ReluUp6_2(self.NormUp6_2(up6_2)) if self.batch_norm else self.ReluUp6_2(up6_2)
    # conv7_1
    merge7_1 = self.Merge7_1([conv1_2, up6_1])
    if self.dropout_rate < 1:
        drop7_1 = self.Drop7_1(merge7_1)
    conv7_1_1 = self.Conv7_1_1(drop7_1)
    conv7_1_1 = self.Relu7_1_1(self.Norm7_1_1(conv7_1_1)) if self.batch_norm else self.Relu7_1_1(conv7_1_1)
    ...
    conv7_1_2 = self.Relu7_1_2(self.Norm7_1_2(conv7_1_2)) if self.batch_norm else self.Relu7_1_2(conv7_1_2)
    # conv7_2
    merge7_2 = self.Merge7_2([conv1_2, up6_2])
    if self.dropout_rate < 1:
        drop7_2 = self.Drop7_2(merge7_2)
    conv7_2_1 = self.Conv7_2_1(drop7_2)
    conv7_2_1 = self.Relu7_2_1(self.Norm7_2_1(conv7_2_1)) if self.batch_norm else self.Relu7_2_1(conv7_2_1)
    ...
    return conv7_1_2, conv7_2_2

if __name__ == "__main__":
    import numpy as np
    import os

    # model = UNnet2H(filters=32, dropout_rate=0.5, batch_norm=True)
    model = UNnetS2H(filters=32, dropout_rate=0.5, batch_norm=True)
