    ...
    def __init__(self,
        ...
        for block_idx in range(self.nstage+1, 2*self.nstage+1):
            ...
            if self.up_type == 'upConv':
                ...
                self.layers_c['upConv{:d}'.format(block_idx)] = Conv2DTranspose(self.filters[block_idx], 2, 2, padding='same', kernel_initializer=kernel_initializer, use_bias=use_bias, kernel_regularizer=l2(weight_decay))
            # merge
